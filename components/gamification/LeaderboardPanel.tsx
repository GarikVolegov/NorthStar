/**
 * LeaderboardPanel.tsx — Phase 3 Gamification
 *
 * Classifica utenti per XP o streak vocale.
 * Evidenzia la riga dell'utente corrente.
 * Switch XP / Streak in alto a destra.
 *
 * Usage:
 *   <LeaderboardPanel />
 *   <LeaderboardPanel limit={10} className="max-w-lg" />
 */
"use client";

import React from "react";
import { useLeaderboard } from "@/lib/api-client-react/src/hooks/useLeaderboard";
import type { LeaderboardEntry } from "@/lib/api-client-react/src/hooks/useLeaderboard";
...
function EntryRow({ entry, showStreak }: { entry: LeaderboardEntry; showStreak: boolean }) {
  ...
  return (
    ...
    >
      {/* Rank */}
      <span className="w-8 text-center text-base shrink-0 font-mono">
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center justify-between gap-1">
          <span className="text-sm font-medium truncate">
            {entry.name}
            {entry.isSelf && (
              <span className="ml-1 text-xs text-primary font-normal">(tu)</span>
            )}
          </span>
          <span className={`text-xs font-semibold shrink-0 ${levelColor(entry.level)}`}>
            Lv.{entry.level}
          </span>
        </div>
  ...
  return (
    <div className={`rounded-2xl border border-border bg-card flex flex-col overflow-hidden ${className}`}>
      ...
      <div className="flex-1 overflow-y-auto px-2 py-2">
        ...
        {error && (
          <p className="text-center text-sm text-red-500 py-8">⚠️ {error}</p>
        )}
        ...
        {/* Self rank footer (when out of top-N) */}
