...
| --- | --- | --- |
| 2026-06-15 | Opus 4.8 | **P1 scalabilità** su branch `feature/p1-scalability` (da `main`): (B1) migrazione `0043_knowledge_nodes_embedding_index.sql` — indice `hnsw` parziale su `knowledge_nodes.embedding_vec` (eliminava il full-scan ad ogni turno Wendy); va applicato manualmente su Neon come le altre migrazioni vettoriali 0035–0042 (il journal Drizzle si ferma a idx 34 — debito noto). (B5) nuovo helper riusabile `cached(key,ttl,fn)` in `lib/redis.ts` applicato a `GET /api/stats/summary` e `GET /api/sectors/:id/stats` (TTL 5min) — erano aggregazioni full-table su `test_sessions` non cacheate e DDoS-abili su endpoint pubblici. (B2) nuovo `lib/singleton-lock.ts` `runWhenLeader()` (lock Redis fail-open) → gli scheduler di background (`startCronJobs`+`startAlertChecker`) girano su un solo leader in multi-istanza (`agentRegistry` resta per-istanza: cache di sola lettura). (B4) store rate-limit Redis di default in prod (`shouldUseRedisRateLimitStore`) + `passOnStoreError:true` → contatori condivisi tra istanze, fail-open su blip Redis; il fail-closed HARD resta gated sul flag esplicito. (B3) fan-out realtime cross-istanza: bridge Redis pub/sub in `ws.ts` (`emitToUser`, canale `ws:emit`) — gli eventi WS raggiungono l'utente anche se connesso a un'altra replica; fatto solo in `apps/server` (ha ioredis) senza toccare `ws-server`; fail-open su consegna locale; `friends.ts` migrato a `emitToUser`. **P1 completo.** Gate verde. Branch P0 sicurezza separato (`feature/p0-security-hardening`). |
