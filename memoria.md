...
- [x] **Schema DB → `drizzle-kit push`** (deciso 2026-06-24, vedi §8): ✅ FATTO (CHORE-002, loop) — `staging.yml`/`production.yml` ora eseguono `db:push` (push non forzato: le modifiche distruttive abortiscono il job invece di applicarle); rimosso lo step `db:migrate:dry-run`. **Resta da validare su DB staging non-prod** prima del merge (il chain SQL è incompleto → `db:migrate` non costruirebbe un DB fresco).
...
### Loop di sviluppo autonomo (Ralph evoluto) — `scripts/ralph/`

Sistema che fa lavorare un'AI in continuazione sul progetto (anche di notte) con **autonomia
bounded**: piena libertà dentro un envelope di sicurezza. Costruito sopra il Ralph esistente
(`ralph.sh`/`prd.json`/`progress.txt`), aggiunge: un **backlog prioritizzato** (`backlog.json`,
policy *security/bug → improvement/chore → feature*), un **runner continuo** (`loop.sh` →
`iterate.prompt.md`), tre **discovery hunter** che riempiono il backlog, e un **self-improvement**
(`PLAYBOOK.md`). La logica critica (selezione + decisione deploy) è **deterministica e testata**
(`lib/*.mjs` + 31 unit test `node --test`, via `pnpm ralph:test`), non lasciata al giudizio del
prompt. Decisioni founder: autonomia totale incl. merge+deploy, substrato locale, costruire le
feature scoperte, bug/sicurezza prima.

**Sicurezza:** il deploy è **gated da segnali oggettivi** (gate verde + `pnpm eval` ≥ KPI
safety/privacy 100% + health-check → altrimenti rollback), non da un gate umano. **Kill-switch:**
`touch scripts/ralph/STOP` mette in pausa. **Guard PreToolUse** (`lib/guard-hook.mjs`) blocca
force-push / `--no-verify` / `rm -rf` di root-home / edit di `.env*` — e regge anche sotto
`--dangerously-skip-permissions` (gli hook non sono bypassati). **Rollout graduale:**
`loop.config.json mode` = `dry-run` → `pr-only` → `full-auto` (parte in `dry-run`). Guida operatore:
[scripts/ralph/README.md](scripts/ralph/README.md); design: [docs/superpowers/specs/2026-06-24-autonomous-loop-design.md](docs/superpowers/specs/2026-06-24-autonomous-loop-design.md).
**Pendente founder:** wiring degli hook in `.claude/settings.json` (la self-modification dell'agente
è gated dall'harness — applicare a mano o approvare l'edit).

**Modalità interattiva — `/ralph-loop` (2026-06-27).** Oltre al runner headless `loop.sh`
(notturno, non presidiato, `claude --print` su file di log) c'è ora `/ralph-loop [max]`:
fa girare il loop **live nella sessione Claude Code** — vedi ogni ragionamento/tool-call/decisione
in tempo reale, con un **checkpoint** compatto tra un'iterazione e l'altra, e lo interrompi
quando vuoi. Stesso core deterministico (`ralph-cli.mjs next`/`decide`) e stesso cervello
per-iterazione (`iterate.prompt.md`); aggiunge il cervello `loop-interactive.prompt.md` e il
...
**Decisione:** staging/prod usano **`drizzle-kit push`** (schema = unica fonte di verità), come già dev/e2e/integration. Si **ritira** `db:migrate` journal-driven. Procedura completa + rischi (primo push su prod esistente → diff distruttivi, usare `--strict`+review+backup) in [`docs/DEPLOY.md`](docs/DEPLOY.md) §2/§7. **Stato (CHORE-002, 2026-06-24):** `staging.yml`/`production.yml` ora eseguono `db:push` (non più `db:migrate`/`db:migrate:dry-run`). **Resta gated su DB non-prod:** validare il push su uno staging prima del merge su `main`. Dettaglio: memoria utente `northstar-drizzle-migration-chain-incomplete`.
...
| --- | --- | --- |
| 2026-06-27 | Opus 4.8 (loop) | **IMP-003 (6° giro live di `/ralph-loop`, pr-only) — indice `session_summaries (userId, createdAt)`.** Il commento dello schema diceva "keyed for fast lookup by userId" ma non c'era indice → seq scan a ogni avvio sessione coach e su `/account/export`. Aggiunto indice composito (via `db:push`, §8 — validazione su DB non-prod gated). Verde: typecheck db + audit db-types. Commit `3fd1480`; PR a mano. |
| 2026-06-27 | Opus 4.8 (loop) | **IMP-002 (5° giro live di `/ralph-loop`, pr-only) — N+1 in `/account/export` + indici FK.** `getUserRelatedData` faceva una SELECT per conversazione (N+1) → query singola `inArray()` (guardia su lista vuota). Aggiunti indici `messages.conversationId` e `conversations.userId` (via `db:push`, §8 — validazione su DB non-prod gated dal founder). Forma risposta invariata. Verde: typecheck db+server, lint, audit. Commit `55884f7`; PR a mano. |
| 2026-06-27 | Opus 4.8 (loop) | **BUG-002 (4° giro live di `/ralph-loop`, pr-only) — chiuso un buco di costo LLM.** Discovery (3° giro: 3 hunter in parallelo) ha rifornito il backlog con 8 item; il top, BUG-002 (P1), era un **buco di abuso reale**: il `costGuard` sommava solo `ai_cost_log` (scritto da `recordAiCall`), ma `roadmap`/`cv`/`briefings` registrano via `recordLlmUsage` → `llm_usage` (tabella che il cost-guard aveva smesso di leggere, pur conservando l'indice composito "cost-guard queries"). Spesa invisibile **e** route non montate col guard → chiamate LLM free illimitate (~4000 token a colpo). Fix: `costGuard` ora somma **entrambi** i ledger (`Promise.all`) ed è montato sui 3 generatori (`cover-letter`/`skills-gap` già gated da `checkMonthlyFreemium`). Test TDD prova la somma (0.30+0.30 > 0.50 free → 403). Verde: server typecheck, lint, 9 audit, 158 server unit. Commit `68fd6ef`; `gh` assente → PR a mano. |
...
| 2026-06-24 | Opus 4.8 | **Loop di sviluppo autonomo (Ralph evoluto) — `scripts/ralph/`.** Brainstorm + spec ([docs/superpowers/specs/2026-06-24-autonomous-loop-design.md](docs/superpowers/specs/2026-06-24-autonomous-loop-design.md)) + implementazione. Sopra il Ralph esistente: **backlog prioritizzato** (`backlog.json`, policy security/bug→improvement/chore→feature), **runner continuo** (`loop.sh`→`iterate.prompt.md`), **3 discovery hunter** (`discovery/*.md`) che riempiono il backlog, **self-improvement** (`PLAYBOOK.md`), **journal** audit per-notte. Logica critica **deterministica + testata** (`lib/{backlog,deploy-gate,safety,journal,guard}.mjs`, **31 unit test** `node --test`, `pnpm ralph:test` verde) — selezione e decisione deploy non lasciate al prompt. **Autonomia bounded** (decisioni founder: autonomia totale incl. merge+deploy, locale, costruire le feature, bug/sicurezza prima): deploy **gated da segnali oggettivi** (gate verde + eval ≥ KPI + health-check → rollback auto), **kill-switch** `scripts/ralph/STOP`, **guard PreToolUse** (`lib/guard-hook.mjs`: blocca force-push/`--no-verify`/`rm -rf` root-home/`.env*`, regge sotto `--dangerously-skip-permissions`), **backoff** anti-thrash. **Rollout** `loop.config.json mode`: `dry-run`→`pr-only`→`full-auto` (parte in `dry-run`). Slash command `/ralph-iterate` `/ralph-discover` `/ralph-status`; script `pnpm ralph:{status,next,test,loop}`. **Pendente founder:** wiring hook in `.claude/settings.json` (self-modification gated dall'harness → approvare/applicare); chiave Clerk test (SEC-001) + DB non-prod (FEAT-001) per sbloccare item; promuovere `mode`. Guida: [scripts/ralph/README.md](scripts/ralph/README.md). |
