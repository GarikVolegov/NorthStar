...
- **Ultima revisione:** 2026-06-18 — *Opus 4.8 (sub-progetto A stabilizzazione 3/3 mergiato; sub-progetto B FE-light: recuperate `/mood` `/diario` `/chi-sono` da `9c9a450` e mergiate; restano Bussola full-stack [in dashboard] + Routines)*
...
**Sub-progetto B — Recupero feature (parziale).** ✅ FATTO e mergiato (FE-light, da `9c9a450`): **`/mood`** (autonomo, localStorage), **`/diario`** (+ `components/diary/*`, rimossa la sotto-feature "milestone certificabile" assente su questa linea), **`/chi-sono`** (+ `features/compass/useCompass.ts`; degrada con grazia finché manca il backend Bussola). Tutte rotte `protected` in `route-config.ts`; verifica browser + cablaggio link di navigazione ancora da fare. **DA FARE (DB-pesante, scoperto più grande del previsto):** **La Bussola full-stack** — `features/compass/*` (BussolaHome/CommittedActionPlan) + 5 pagine + router `compass.ts`+`spikes.ts` + **4 tabelle nuove** (`compassProfile`, `compassSignal`, `sceneCard`, `careerSpike`) + migrazioni, **integrata DENTRO la dashboard** (non pagina a parte). **C:** Ritual Engine `/routines` (route `routines.ts`+`monthly-ritual.ts` + 3 tabelle + cron). Entrambi toccano la riconciliazione del journal Drizzle (§8). Fonte di recupero: commit `9c9a450`.
