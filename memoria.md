...
- **Ultima revisione:** 2026-06-17 — *Opus 4.8 (/ralph `ralph/stabilize-current-line` 3/3: fix login [chiave Clerk non esposta a Vite], deploy [gate fuori dal build Vercel], news [fallback statico]; scoperta la divergenza prod↔linea-attuale)*
...
**Sub-progetti B/C — Recupero feature (DA FARE, deciso col founder).** B (solo FE): `chi-sono`, `diario`+`mood`, **La Bussola integrata DENTRO la dashboard** (non pagina a parte). C (full-stack): Ritual Engine `/routines` (+ tabelle/migrazioni/cron). Fonte di recupero: commit `9c9a450`.
