  ...
  if (avatarUrl) {
    return (
      <img
        ...
        height={80}
        loading="lazy"
        className={`${dim} rounded-full object-cover ring-4 ring-foreground/10 flex-shrink-0`}
      />
    );
  }
  return (
    <div className={`${dim} rounded-full bg-[hsl(var(--muted))] flex items-center justify-center flex-shrink-0 ring-4 ring-foreground/10`}>
      <span className="font-bold text-[hsl(var(--chart-3))]">{initials}</span>
    </div>
  );
}
...
function JourneyBadge({ type }: { type: string | null }) {
  if (!type) return null;
  const b = journeyLabels[type] ?? { label: type, color: 'text-[hsl(var(--muted-foreground))] bg-foreground/5 border-foreground/10' };
  return (
    ...
    <div className="max-w-xl mx-auto px-4 py-8 space-y-6 animate-pulse">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1 space-y-3">
          ...
          <div className="h-3.5 w-28 bg-[hsl(var(--muted))] rounded" />
          <div className="h-7 w-24 bg-[hsl(var(--muted))] rounded-full" />
        </div>
      </div>
      <div className="h-10 w-full bg-[hsl(var(--muted))] rounded-xl" />
      <div className="grid grid-cols-3 gap-3">
        {[0,1,2].map((i) => <div key={i} className="h-16 bg-[hsl(var(--muted))] rounded-xl" />)}
      </div>
      <div className="space-y-2">
        <div className="h-3.5 w-full bg-[hsl(var(--muted))] rounded" />
        <div className="h-3.5 w-4/5 bg-[hsl(var(--muted))] rounded" />
      </div>
    </div>
  );
}
  ...
  if (status === 'pending_sent') return (
    <div className="flex gap-2">
      <span className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm
                       font-medium text-[hsl(var(--muted-foreground))] bg-[hsl(var(--background))] border border-foreground/[0.08]">
      ...
      <button
        ...
        className="px-3 py-2.5 rounded-xl text-sm text-[hsl(var(--muted-foreground))] border border-foreground/[0.08]
    ...
    <div className="flex gap-2">
      <button
        ...
        className="flex-1 flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl
                   text-sm font-semibold text-[hsl(var(--foreground))] bg-[hsl(var(--muted))] hover:bg-[hsl(var(--muted))]
    ...
    <div className="flex gap-2">
      ...
      <Link
        ...
        className="flex items-center justify-center gap-2 flex-1 px-4 py-2.5 rounded-xl text-sm
                   font-semibold text-[hsl(var(--chart-3))] bg-[hsl(var(--muted))] hover:bg-[hsl(var(--muted))]
                   transition-colors"
...
function StatPill({
  ...
  return (
    <div className="flex flex-col items-center gap-1 p-4 rounded-xl bg-[hsl(var(--background))] border border-foreground/[0.06]">
      <Icon className="w-4 h-4 text-[hsl(var(--muted-foreground))]" />
      <span className="text-[18px] font-bold text-[hsl(var(--foreground))] tabular-nums">{value}</span>
...
function BadgeCard({ badge }: { badge: {
  ...
  return (
    <div
      className="flex flex-col items-center gap-2 p-3 rounded-xl bg-[hsl(var(--background))]
    ...
    >
      ...
      <span className="text-[11px] font-medium text-[hsl(var(--muted-foreground))] leading-tight line-clamp-2">
        {badge.name}
      </span>
    </div>
  );
...
export default function ProfilePage() {
  ...
  if (error) {
    ...
    return (
      <div className="max-w-xl mx-auto px-4 py-16 flex flex-col items-center text-center gap-4">
        ...
        <h2 className="text-[17px] font-semibold text-[hsl(var(--foreground))]">
          {isPrivate ? 'Profilo privato' : 'Profilo non trovato'}
        </h2>
        <p className="text-[13px] text-[hsl(var(--muted-foreground))] max-w-[26ch]">
          {isPrivate
        ...
        <Link
          href="/amici"
          className="mt-2 px-4 py-2 rounded-lg bg-[hsl(var(--muted))] text-[hsl(var(--chart-3))] text-sm font-medium
  ...
  return (
    ...
    >
      ...
      <Link
        href="/amici"
        className="inline-flex items-center gap-1.5 text-[13px] text-[hsl(var(--muted-foreground))]
      ...
      {/* ── Hero card ──────────────────────────────────────────────────── */}
      <div className="p-6 rounded-2xl bg-[hsl(var(--background))] border border-foreground/[0.06] space-y-5">

        {/* Avatar + nome + settore */}
        <div className="flex items-start gap-4">
          <Avatar name={user.name} avatarUrl={user.avatarUrl} size="xl" />
          <div className="flex-1 min-w-0">
            <h1 className="text-xl font-bold text-[hsl(var(--foreground))] leading-tight">{user.name}</h1>
            {user.sectorName && (
              <p className="text-[13px] text-[hsl(var(--muted-foreground))] mt-0.5">{user.sectorName}</p>
            )}
        ...
        {user.isOwner ? (
          <Link
            ...
            className="flex items-center justify-center gap-2 w-full px-4 py-2.5 rounded-xl
                       text-sm font-medium text-[hsl(var(--muted-foreground))] border border-foreground/[0.08]
        ...
        {/* Bio */}
        {user.bio && (
          <p className="text-[13px] text-[hsl(var(--muted-foreground))] leading-relaxed border-t border-foreground/[0.06] pt-4">
            {user.bio}
          </p>
        )}
      </div>
      ...
      {badges.length > 0 && (
        <section>
          <h2 className="text-[13px] font-semibold text-[hsl(var(--muted-foreground))] uppercase tracking-wider mb-3">
            Badge
          </h2>
          <div className="grid grid-cols-3 gap-3">
            {badges.map((b) => (
      ...
      {user.linkedinUrl && (
        <a
          ...
          className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl
                     text-sm font-medium text-[hsl(var(--chart-3))] border border-[hsl(var(--muted))]/60
