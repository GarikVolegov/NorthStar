 ...
 *   FIRECRAWL_BASE_URL  — default http://firecrawl-api:3002 (in docker network)
 *   FIRECRAWL_API_KEY   — opzionale in self-host puro
 *   FIRECRAWL_TIMEOUT_MS — default 30000
 *
 * Non aggiunge dipendenze (usa fetch nativo Node >=18).
 * Retry leggero (1 tentativo) su errori di rete; nessun retry su 4xx.
 */
...
export class FirecrawlClient {
  ...
  /** Disponibilità del servizio (per health/feature-flag). */
  isAvailable(): boolean {
  ...
  /** Scrape singolo URL → markdown + metadata. */
  ...
  /**
   * Avvia un crawl asincrono. Ritorna l'id del job (da pollare con getCrawlStatus).
   */
  ...
  /**
   * Crawl bloccante: polla fino a completamento o timeout.
   * Utile per ingestion batch via route admin.
   */
  async crawlAndWait(
    url: string,
    ...
    const maxWaitMs  = pollOptions.maxWaitMs  ?? 300_000; // 5 min default
  ...
  /** Map: scopre URL di un sito senza scrapare i contenuti. */
  ...
  /** Search web → risultati eventualmente scrappati a markdown. */
  ...
  /** Extract dati strutturati via LLM da uno o più URL. */
