...
const MAX_MARKDOWN_CHARS = 12_000;       // ~3000 token, evita di saturare il context
    ...
    return {
      ...
      data: {
        ...
        results: results.map((r) => ({
          ...
          markdown:    r.markdown ? truncate(r.markdown, MAX_MARKDOWN_CHARS / Math.max(results.length, 1)) : undefined,
        })),
