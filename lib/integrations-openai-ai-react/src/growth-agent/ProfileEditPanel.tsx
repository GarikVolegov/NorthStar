/**
 * ProfileEditPanel v2 — aggiunta sezione RIASEC (Passo 2).
 *
 * CHANGES vs v1:
 * - Sezione 5 'Il tuo profilo RIASEC' aggiunta come ultimo accordion
 * - RiasecProfileCard montata con lazy-load (fetch solo al primo open)
 * - onRetakeTest prop opzionale per navigare al test dal profilo
 */
...
function RadioCard({ value, selected, emoji, label, description, onClick }: {
...
function AvatarUploader({ name, avatarUrl, onUpload }: {
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try { await onUpload(file); } catch (err) { setUploadErr(err instanceof Error ? err.message : "Errore"); }
...
export interface ProfileEditPanelProps {
  ...
  onRetakeTest?:  () => void;  // naviga alla pagina del test RIASEC
  className?:     string;
}
...
export function ProfileEditPanel({
  ...
  const [openSections, setOpenSections] = useState<Record<string, boolean>>({
    identity: true, journey: true, preferences: false, cv: false, riasec: false,
  });
  // Lazy: monta RiasecCard solo dopo il primo open
  const [riasecMounted, setRiasecMounted] = useState(false);

  function toggleSection(key: string) {
    setOpenSections((s) => ({ ...s, [key]: !s[key] }));
    if (key === "riasec" && !openSections.riasec) setRiasecMounted(true);
  }
    ...
    <div className={`space-y-4 max-w-2xl mx-auto ${className}`}>
      ...
      <SectionAccordion title="CV / Background" emoji="📎" open={openSections.cv} onToggle={() => toggleSection("cv")}>
        <p className="text-xs text-muted-foreground">
          ...
          Wendy lo usa per darti consigli più precisi. L’IA estrae automaticamente le informazioni chiave in background.
        </p>
