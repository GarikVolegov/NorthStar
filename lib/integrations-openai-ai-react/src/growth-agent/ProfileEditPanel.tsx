/**
 * ProfileEditPanel — Passo 1 del profilo utente NorthStar.
 *
 * SEZIONI:
 *   1. Identità        — nome, avatar, timezone
 *   2. Il tuo percorso — journeyType (radio card), userMode (radio card)
 *   3. Preferenze     — workPreference, autonomy/stability sliders
 *   4. CV / Bio        — textarea cvText con estrazione AI in background
 *
 * Design:
 *   Ogni sezione è un accordion collassabile con header cliccabile.
 *   Salvataggio ottimistico: il bottone mostra 'Salvato ✓' per 2s.
 *   Errori inline senza perdere il contenuto.
 *
 * Props:
 *   token          JWT per le chiamate API
 *   initialData?   Pre-popola il form (evita fetch extra se il parent li ha già)
 *   apiBase?       Default '/api'
...
const JOURNEY_OPTIONS = [
  {
    value:       "indeciso",
    label:       "Ancora in esplorazione",
    description: "Non ho chiaro il percorso, sto cercando direzione",
    emoji:       "🧭",
  },
  {
    value:       "in_transizione",
    label:       "In transizione",
    description: "Sto cambiando settore, ruolo o stile di vita",
    emoji:       "🔄",
  },
  {
    value:       "in_crescita",
    label:       "In crescita attiva",
    description: "Ho una direzione chiara, voglio accelerare",
    emoji:       "🚀",
  },
  {
    value:       "autonomo",
    label:       "Indipendente / Imprenditore",
    description: "Costruisco qualcosa di mio, voglio scalare",
    emoji:       "🏗️",
  },
];
...
const MODE_OPTIONS = [
  {
    value:       "explorer",
    label:       "Explorer",
    description: "Voglio esplorare opzioni e scoprire cosa mi piace",
    emoji:       "🔭",
  },
  {
    value:       "builder",
    label:       "Builder",
    description: "Voglio costruire competenze e progetti concreti",
    emoji:       "🛠️",
  },
  {
    value:       "achiever",
    label:       "Achiever",
    description: "Voglio raggiungere obiettivi misurabili e veloci",
    emoji:       "🏆",
  },
];
...
function useProfileEdit(token: string, apiBase: string, initialData?: Partial<ProfileData>) {
  ...
  // Fetch profile if not pre-populated
  React.useEffect(() => {
    ...
    try {
      const res = await fetch(`${apiBase}/users/me`, {
        ...
        body:    JSON.stringify({
          ...
          autonomyPreference:  form.autonomyPreference,
          stabilityPreference: form.stabilityPreference,
          cvText:              form.cvText,
      ...
      setStatus("error"); setError(e instanceof Error ? e.message : "Errore salvataggio");
    }
  }, [form, token, apiBase]);
...
function RadioCard({
  value, selected, emoji, label, description, onClick,
}: {
  value: string; selected: boolean; emoji: string;
...
function SliderField({
  ...
  label: string; sublabelLeft: string; sublabelRight: string;
...
function AvatarUploader({
  ...
  async function handleFile(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (!file) return;
    if (file.size > 2 * 1024 * 1024) { setUploadErr("Max 2MB"); return; }
    setUploading(true); setUploadErr("");
    try { await onUpload(file); }
    catch (err) { setUploadErr(err instanceof Error ? err.message : "Errore"); }
...
export function ProfileEditPanel({
  ...
  return (
    <div className={`space-y-4 max-w-2xl mx-auto ${className}`}>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs text-muted-foreground mt-0.5">
            Queste informazioni guidano Wendy nelle risposte
          </p>
        </div>
        <button
          onClick={handleSave}
        ...
        <div className="space-y-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-1">
            {JOURNEY_OPTIONS.map((opt) => (
              <RadioCard key={opt.value} {...opt}
                selected={form.journeyType === opt.value}
        ...
        <div className="space-y-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mt-1">
            {MODE_OPTIONS.map((opt) => (
              <RadioCard key={opt.value} {...opt}
                selected={form.userMode === opt.value}
      ...
      {/* Section 3 — Preferenze */}
      <SectionAccordion
        title="Preferenze lavorative" emoji="⚙️"
      ...
      >
        ...
        <SliderField
          label="Autonomia vs Struttura"
          sublabelLeft="Max struttura"
          sublabelRight="Max autonomia"
          value={form.autonomyPreference}
          onChange={(v) => update("autonomyPreference", v)}
        />

        <SliderField
          label="Stabilità vs Crescita rapida"
          sublabelLeft="Stabilità"
          sublabelRight="Crescita aggressiva"
          value={form.stabilityPreference}
          onChange={(v) => update("stabilityPreference", v)}
        />
      </SectionAccordion>

      {/* Section 4 — CV / Bio */}
      <SectionAccordion
        title="CV / Background" emoji="📎"
        open={openSections.cv}
        onToggle={() => toggleSection("cv")}
      >
        <p className="text-xs text-muted-foreground">
          Incolla il tuo CV, bio o esperienza professionale in testo libero.
          Wendy lo usa per darti consigli più precisi. L’IA estrae automaticamente
          le informazioni chiave in background.
        </p>
        <textarea
          rows={8}
          value={form.cvText ?? ""}
          onChange={(e) => update("cvText", e.target.value || null)}
          placeholder="Es. Sono uno sviluppatore con 3 anni di esperienza in React e Node.js. Ho lavorato in startup fintech. Mi interessa il product management..."
        ...
        {form.cvText && form.cvText.length > 50 && (
          <p className="text-[10px] text-muted-foreground flex items-center gap-1">
            ✨ Dopo il salvataggio Wendy analizza il testo e ne estrae le competenze chiave
          </p>
        )}
      </SectionAccordion>
      ...
      <div className="pb-8">
        ...
        >
          {status === "saving" ? "Salvataggio in corso..." :
           status === "saved"  ? "✓ Profilo salvato" : "Salva modifiche"}
        </button>
      </div>
    </div>
  );
