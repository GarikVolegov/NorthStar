---
name: pp-sentry
description: "Printing Press CLI for Sentry. Sentry Public API"
author: "Garik V."
license: "Apache-2.0"
...
Run `sentry-pp-cli auth setup` for the URL and steps to obtain a token (add `--launch` to open the URL). Then store it:

```bash
sentry-pp-cli auth set-token YOUR_TOKEN_HERE
```

Or set `REFERENCE_AUTH_TOKEN` as an environment variable.
