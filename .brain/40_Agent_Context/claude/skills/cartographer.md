...
- `agent-kit-sync`: compare `AGENTS.md`, `.brain/40_Agent_Context/AGENT_CONTEXT.md`, `.brain/40_Agent_Context/claude/skills/`, `.brain/40_Agent_Context/claude/agents/`, hook config, and `.brain/30_Process/Agent-Development-Kit.md`; propose alignment diffs only.

## Inputs

- Commit range, usually `HEAD~1..HEAD` or the current phase branch range.
- Current `.brain/30_Process/Planning/phases/**/PLAN.md`.
- Changed files from `git diff --name-only`.
- `.brain/_ROOT_MOC.md`, layer MOCs, and relevant subsystem/process notes.
- `AGENTS.md`, `.brain/40_Agent_Context/AGENT_CONTEXT.md`, `.brain/40_Agent_Context/claude/skills/*.md`, `.brain/40_Agent_Context/claude/agents/*.md`, `.brain/40_Agent_Context/claude/settings.json`, and `.brain/40_Agent_Context/claude/hooks/*.ps1` when the Agent Development Kit changes.
...
- Do not edit generated Graphify output; it is rebuilt on demand and ignored by Git.
- Do not edit active git worktrees; check `git worktree list --porcelain` first.
