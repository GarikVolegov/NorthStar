...
- Accesso alle env vars di sviluppo, oppure `.env.local` derivato dal template versionato `.env`.
...
`pnpm dev` avvia Docker, API Express e frontend Vite. Se Docker non e' disponibile, avvia Postgres/Redis manualmente e verifica `DATABASE_URL` e `REDIS_URL` in `.env.local`.
...
Segui `.brain/40_Agent_Context/rules/GIT_RULES.md` per naming e flusso completo.
...
- [ ] Ho letto il RULES file dell'area modificata (`.brain/40_Agent_Context/rules/API_RULES.md`, `.brain/40_Agent_Context/rules/DB_RULES.md`, `.brain/40_Agent_Context/rules/FRONTEND_RULES.md`, `.brain/40_Agent_Context/rules/AI_RULES.md`, `.brain/40_Agent_Context/rules/GIT_RULES.md`).
...
- Architettura e flussi: `.brain/20_Product/ARCHITECTURE.md`.
- Incident e rollback: `.brain/30_Process/RUNBOOK.md`.
- Security disclosure pubblica: `.brain/30_Process/SECURITY.md`.
- Regole interne security: `.brain/40_Agent_Context/rules/SECURITY_RULES.md`.
