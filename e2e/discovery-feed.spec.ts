...
const E2E_USER_PASSWORD = process.env.E2E_USER_PASSWORD ?? 'E2ePassword123!';
...
async function goToFeed(page: import('@playwright/test').Page) {
  ...
  await expect(page.getByTestId('discovery-feed-page')).toBeVisible({ timeout: 8_000 });
}
...
test.describe('Discovery Feed', () => {
  ...
  test('rendering card: almeno una card visibile al caricamento', async ({ page }) => {
    await goToFeed(page);
    // Attende il primo render delle card (possibile loading state)
    await expect(page.getByTestId('feed-loading')).not.toBeVisible({ timeout: 8_000 });
  ...
  test('bookmark su una card → appare nella sezione salvati', async ({ page }) => {
    await goToFeed(page);
    await expect(page.getByTestId('feed-loading')).not.toBeVisible({ timeout: 8_000 });
    ...
    await expect(page.getByTestId('saved-item').first()).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('filtro per categoria aggiorna il feed', async ({ page }) => {
    ...
    await expect(page.getByTestId('feed-card').first()).toBeVisible({ timeout: 5_000 });
  });
  ...
  test('apertura dettaglio card', async ({ page }) => {
    ...
    await page.getByTestId('feed-card').first().getByTestId('feed-card-open-btn').click();
    // Il dettaglio deve essere visibile — modal o nuova pagina
    await expect(
      page.getByTestId('feed-item-detail')
