...
test.describe('CV Builder', () => {
  ...
  test('compilazione dati personali e salvataggio', async ({ page }) => {
    ...
    await page.getByTestId('cv-input-summary').fill(
  ...
  test('aggiunta esperienza lavorativa', async ({ page }) => {
    ...
    await page.getByTestId('cv-exp-description').fill(
      'Sviluppo interfacce React, TypeScript, testing con Vitest.'
    );
  ...
  test('download PDF avvia il download', async ({ page }) => {
    ...
    await cvList.getByTestId('cv-list-item').first().getByTestId('cv-download-btn').click();
    const download = await downloadPromise;
    expect(download.suggestedFilename()).toMatch(/\.pdf$/i);
  });

  test('validazione: salva senza dati obbligatori mostra errore inline', async ({ page }) => {
    await page.getByTestId('nav-cv-builder').click();
    await page.getByTestId('cv-create-new-btn').click();
    ...
    await expect(page.getByTestId('cv-validation-error').first()).toBeVisible({ timeout: 3_000 });
  });
});
