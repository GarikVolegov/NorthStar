...
import { CvEditorDrawer, type GeneratedCv } from "./cv/CvEditorDrawer";
...
type CvListResponse = { cvs?: CvMeta[] };
type CvMutationResponse = { success?: boolean; cvs?: CvMeta[] };
...
export function CvSection({ userId }: { userId: number }) {
  ...
  const [selectedTemplate, setSelectedTemplate] = useState<CvTemplate>(() => {
    try {
      return (localStorage.getItem(LS_KEY) as CvTemplate) ?? "classic";
    } catch {
      return "classic";
    }
  });
  useEffect(() => {
    try {
      localStorage.setItem(LS_KEY, selectedTemplate);
    } catch {
      return;
    }
  }, [selectedTemplate]);
  ...
  async function openEditor() {
    ...
    try {
      const j = await getJson<CvDetailResponse>(`${BASE}api/cv/${userId}`);
      const gen = j.cvData?.generated ?? null;
      if (gen) {
        setGeneratedCvData(gen);
        setEditorOpen(true);
      }
    } catch {
      return;
  ...
  const uploadMutation = useMutation({
    mutationFn: async (file: File) => {
      ...
      return postJson<CvMutationResponse>(`${BASE}api/cv/mine/upload`, {
        fileDataUrl: dataUrl,
        filename: file.name,
        mimeType: file.type,
      });
  ...
  function handleFileChange(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    if (file.size > 5_000_000) {
      setUploadError("File troppo grande (max 5 MB)");
      return;
    }
    if (!["application/pdf", "text/plain"].includes(file.type)) {
      setUploadError("Solo PDF o TXT");
      return;
    }
    setUploadError(null);
  ...
  const generateMutation = useMutation({
    mutationFn: async (template: CvTemplate) => {
      return postJson<CvMutationResponse>(`${BASE}api/cv/mine/generate`, {
        template,
      });
    },
    onSuccess: () => {
    ...
    onError: (err: unknown) =>
      setGenerateError(apiErrorMessage(err, "Errore generazione")),
  });

  // ── Delete
  const deleteMutation = useMutation({
    mutationFn: async () => {
      await deleteJson(`${BASE}api/cv/mine`);
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["cvs-mine", userId] });
    },
      ...
      <Card className="rounded-2xl">
        ...
        <CardContent className="space-y-4">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    ...
                    {generatedCv.template && (
                      <span
                        className={cn(
                          "ml-1.5 capitalize font-medium",
                          activeTpl.accent,
                        )}
                      >
                        ·{" "}
                        {TEMPLATES.find((t) => t.id === generatedCv.template)
                          ?.label ?? generatedCv.template}
                      </span>
                    )}
                  </p>
                </div>
            ...
            >
              <div className="flex items-start gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-muted-foreground">
                    Caricato · {formatDate(uploadedCv.uploadedAt)}
                  </p>
                </div>
              </div>
              {!generatedCv && (
                  ...
                  >
                    <div className="w-10 h-14 rounded-md border border-border overflow-hidden flex flex-col">
                      ...
                      <div className="flex-1 flex flex-col gap-0.5 p-1">
                        <div
                          className={cn("h-0.5 w-full rounded-full", tpl.dot2)}
                        />
          ...
          {generateMutation.isSuccess && (
            <p className="flex items-center gap-1.5 text-xs text-emerald-700 bg-emerald-50 rounded-lg px-3 py-2">
              <CheckCircle2 className="w-3.5 h-3.5" /> CV generato con template{" "}
              {activeTpl.label}!
            </p>
          )}
          {uploadError && (
