...
import {
  ...
  Sparkles, Target, TrendingUp, Trophy, User, Users, GraduationCap, Loader2, Zap, Lock, DollarSign, X,
} from "lucide-react";
...
export function UserDashboard({ userName, latestResult, isPremium = false }: UserDashboardProps) {
  ...
  return (
    <div className="flex flex-col w-full animate-in fade-in duration-500">
            ...
            <div className="flex items-start gap-4 p-5 rounded-2xl border border-primary/20 bg-background">
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 mb-3">
                  {[
                    { num: "1", label: "Fai il test RIASEC", href: "/test" },
                ...
                <button
                  onClick={() => { localStorage.setItem("ns_onboarding_done", "1"); setOnboardingDone(true); }}
      ...
      {/* AI Professions — shown when agent data is ready */}
      {(agentLoading || agentProfessions.length > 0) && (
            ...
            <div className="flex items-center justify-between mb-5">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-0.5">
                  {agentLoading ? "L'AI sta elaborando il tuo profilo…" : `${agentProfessions.length} professioni · Piano ${isPremiumAgent ? "Premium" : "gratuito"}`}
                </p>
              </div>
              <Link href="/dashboard">
                <div className="inline-flex items-center gap-1.5 text-sm text-primary font-medium hover:underline">
            ...
            {agentLoading ? (
              <div className="space-y-3">
                <div className="flex items-center gap-2 text-primary animate-pulse mb-3">
                  ...
                  <span className="text-xs font-medium">Analisi AI in corso — potrebbe richiedere alcuni secondi…</span>
                </div>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {[1, 2, 3].map((i) => (
