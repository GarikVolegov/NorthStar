      ...
      {loading ? (
        ...
        <div className="space-y-5">
          <div className="grid grid-cols-2 lg:grid-cols-6 gap-3">
            {[
              { label: "Utenti", value: data.business.users.total, detail: `+${data.business.users.new30d} in 30g` },
              { label: "Nuovi 7g", value: data.business.users.new7d, detail: "Acquisizione breve" },
              { label: "Test", value: data.business.tests.total, detail: `+${data.business.tests.recent30d} in 30g` },
              { label: "Premium", value: data.business.users.premium, detail: `${data.business.users.conversionRate}% conversione` },
              { label: "Test/User", value: `${data.funnels.userToTestRate}%`, detail: "Attivazione" },
              { label: "Lead conv.", value: `${data.funnels.leadConversionRate}%`, detail: "Partner" },
            ].map((item) => (
          ...
          <div className="grid lg:grid-cols-2 gap-4">
            <div className="rounded-lg border bg-card p-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="rounded-lg bg-muted/40 p-3">
                  ...
                  <p className="text-lg font-bold">{data.business.users.new7d} / {data.business.users.new30d}</p>
                </div>
                <div className="rounded-lg bg-muted/40 p-3">
                  <p className="text-xs text-muted-foreground">Test 7g / 30g</p>
                  <p className="text-lg font-bold">{data.business.tests.recent7d} / {data.business.tests.recent30d}</p>
                </div>
                <div className="rounded-lg bg-muted/40 p-3">
                  <p className="text-xs text-muted-foreground">Test confermati</p>
                  <p className="text-lg font-bold">{data.business.tests.completionRate}%</p>
                </div>
                <div className="rounded-lg bg-muted/40 p-3">
            ...
            <div className="rounded-lg border bg-card p-4">
              <div className="flex items-center justify-between gap-2 mb-3">
                ...
                <Badge variant={data.technical.status === "healthy" ? "secondary" : "default"}>
                  {data.technical.label}
                </Badge>
              </div>
              <div className="space-y-2">
                {data.actions.filter((action) => (action.count ?? 0) > 0).slice(0, 5).map((action) => (
                  <button
                    key={action.path}
                    type="button"
            ...
            <div className="rounded-lg border bg-card p-4">
              ...
              <div className="space-y-1">
                {data.business.topSectors.slice(0, 5).map((sector) => (
                  <div key={String(sector.sectorId)} className="flex items-center justify-between p-2 rounded-lg bg-muted/40">
                    <span className="text-sm">Settore #{sector.sectorId ?? "n/d"}</span>
