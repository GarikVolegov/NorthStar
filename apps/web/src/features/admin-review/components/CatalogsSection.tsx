...
export function CatalogsSection({
  ...
  const catalogOverviewByType = new Map(
    catalogs.overview.map((item) => [item.type, item]),
  );
    ...
    <div className="p-4 md:p-8 space-y-5">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            Editing controllato con bozze, preview utente, pubblicazione e
            audit.
          </p>
        </div>
          ...
          return (
            ...
            >
              ...
              <p className="text-xs text-muted-foreground">
                {overview
                  ? `${overview.active} attivi · ${overview.archived} archiviati`
                  : "Caricamento"}
              </p>
            </button>
          );
      ...
      <div className="flex flex-col gap-3 lg:flex-row">
        ...
        >
          ...
          {catalogs.type === "growth_articles" && (
            <option value="draft">Draft articoli</option>
          )}
          {catalogs.type === "growth_articles" && (
      ...
      {Object.keys(catalogs.fields).length > 0 && (
        <div
          className="rounded-md border border-danger-muted bg-danger-surface p-3 text-sm text-danger"
          role="alert"
        >
          <p className="font-semibold">
            Correggi questi problemi prima di continuare:
          </p>
          ...
          <div className="border-b px-4 py-3 flex items-center justify-between">
            <div>
              <p className="text-sm font-semibold">
                {catalogs.data?.label ?? "Catalogo"}
              </p>
              <p className="text-xs text-muted-foreground">
                {catalogs.data?.items?.length ?? 0} elementi ·{" "}
                {catalogs.data?.drafts?.length ?? 0} bozze aperte
              </p>
            </div>
            {catalogs.loading && (
                    ...
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0">
                        <p className="font-medium text-sm truncate">
                          {catalogTitle(catalogs.type, item)}
                        </p>
          ...
          <div className="rounded-md border bg-card">
            <div className="border-b px-4 py-3 flex flex-col gap-2 lg:flex-row lg:items-center lg:justify-between">
              <div>
                <p className="text-sm font-semibold">
                  {catalogs.selected
                    ? catalogTitle(catalogs.type, catalogs.selected)
                    : "Nuova bozza"}
                </p>
                <p className="text-xs text-muted-foreground">
                  {catalogs.draftId
                    ? `Bozza #${catalogs.draftId} pronta per pubblicazione`
                    : "Salva una bozza prima di pubblicare un nuovo contenuto."}
                </p>
              </div>
              <div className="flex flex-wrap gap-2">
                <Button
                  ...
                  variant="outline"
                  className="min-h-11"
                  onClick={() => void catalogs.runAction("preview")}
                  disabled={catalogs.actionLoading === "preview"}
                >
                ...
                <Button
                  size="sm"
                  className="min-h-11"
                  onClick={() => void catalogs.runAction("draft")}
                  disabled={catalogs.actionLoading === "draft"}
                >
                ...
                <Button
                  size="sm"
                  className="min-h-11"
                  onClick={() => void catalogs.runAction("publish")}
                  disabled={catalogs.actionLoading === "publish"}
                >
            ...
            <div className="p-4 space-y-3">
              ...
              <textarea
                ...
                onChange={(event) =>
                  catalogs.setPayloadText(event.target.value)
                }
                spellCheck={false}
              />
            </div>
              ...
              <div className="border-t px-4 py-3 flex flex-wrap gap-2">
                {isCatalogArchived(catalogs.type, catalogs.selected) ? (
                  <Button
                    ...
                    variant="outline"
                    className="min-h-11"
                    onClick={() => void catalogs.runAction("restore")}
                    disabled={catalogs.actionLoading === "restore"}
                  >
                  ...
                  <Button
                    ...
                    onClick={() => void catalogs.runAction("archive")}
                    disabled={catalogs.actionLoading === "archive"}
                  >
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            <div className="rounded-md border bg-card p-4">
              ...
              {catalogs.preview ? (
                <div className="rounded-md border bg-background p-4">
                  <p className="text-xs text-muted-foreground">
                    {catalogs.preview.subtitle}
                  </p>
                  <h4 className="mt-1 font-serif text-lg font-bold">
                    {catalogs.preview.title}
                  </h4>
                  <p className="mt-2 text-sm text-muted-foreground">
                    {catalogs.preview.description}
                  </p>
                ...
                <p className="text-sm text-muted-foreground">
                  Genera una preview per vedere come apparira agli utenti prima
                  della pubblicazione.
                </p>
              )}
            </div>
            ...
            <div className="rounded-md border bg-card p-4">
              ...
              {catalogs.auditTrail.length > 0 ? (
                ...
                <p className="text-sm text-muted-foreground">
                  Le decisioni su questo elemento appariranno qui.
                </p>
              )}
            </div>
          </div>
