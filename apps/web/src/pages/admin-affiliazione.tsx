...
export default function AdminAffiliazione() {
  ...
  const typeCounts = Object.keys(PARTNER_LABELS).reduce<Record<string, number>>((acc, k) => {
    ...
    <AdminAuthGate title="Admin Affiliazione" description="Gestisci le richieste di affiliazione">
      ...
      <div className="sticky top-0 z-30 border-b bg-background/90 backdrop-blur">
        <div className="container mx-auto px-4 max-w-5xl flex h-14 items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" className="rounded-full gap-1.5" onClick={() => fetchLeads(key)} disabled={loading}>
      ...
      <div className="container mx-auto px-4 max-w-5xl py-8 space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {[
            { label: "Totali",     value: leads.length,  color: "text-foreground" },
        ...
        <div className="rounded-2xl border bg-card p-4 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            <button
              ...
              className={cn("px-3 py-1.5 rounded-xl border text-xs font-medium transition-colors",
                filterType === "all" ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/40")}
            >
              Tutti ({leads.length})
            </button>
            {Object.entries(PARTNER_LABELS).map(([k, label]) =>
              typeCounts[k] > 0 && (
                <button key={k} onClick={() => setFilterType(k)}
                  className={cn("px-3 py-1.5 rounded-xl border text-xs font-medium transition-colors",
                    filterType === k ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/40")}
                >
                  {label} ({typeCounts[k]})
                </button>
              )
          ...
          <div className="flex flex-wrap items-center gap-2 pt-1 border-t">
            {(["all", "nuovo", "contattato", "in_trattativa", "attivo"] as const).map(v => (
              <button key={v} onClick={() => setFilterStatus(v)}
                className={cn("px-3 py-1.5 rounded-xl border text-xs font-medium transition-colors",
                  filterStatus === v ? "border-primary bg-primary/10 text-primary" : "border-border text-muted-foreground hover:border-primary/40")}
              >
                ...
                <div className="flex items-start gap-3 p-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex flex-wrap items-center gap-2 mb-1">
                      <span className="font-semibold text-sm">{lead.institutionName}</span>
                      <span className={cn("text-xs px-2 py-0.5 rounded-full font-medium",
                        PARTNER_COLORS[lead.partnerType] ?? PARTNER_COLORS["altro"])}>
                    ...
                    <div className="flex flex-wrap items-center gap-3 text-xs text-muted-foreground">
                      ...
                      {lead.estimatedUsers && (
                        <span className="flex items-center gap-1">
                          <Users className="w-3 h-3" /> {lead.estimatedUsers} utenti
                        </span>
                      )}
                    </div>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    {!lead.read && (
                      <Button size="sm" variant="outline" className="h-8 rounded-full text-xs gap-1.5"
                        onClick={() => markContacted(lead.id)}>
                        <CheckCheck className="w-3.5 h-3.5" /> Segna contattato
                      </Button>
                    )}
                    <button
                  ...
                  <div className="border-t bg-muted/30 px-5 py-4 space-y-4">
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                      <div className="p-3 rounded-xl bg-background border">
                        ...
                        <p className="text-sm font-medium">{PARTNER_LABELS[lead.partnerType] ?? lead.partnerType}</p>
                      </div>
                      <div className="p-3 rounded-xl bg-background border">
                        <p className="text-xs text-muted-foreground mb-1 flex items-center gap-1">
