...
export default function CrescitaCategoria() {
  ...
  const { data, isLoading } = useQuery<{ articles: Article[]; total: number }>({
    ...
    queryFn: () => fetch(`${BASE}api/crescita?category=${cat}&limit=50`).then(r => r.json()),
    staleTime: 1000 * 60 * 5,
    enabled: !!cat,
  });
  ...
  usePageMeta({
    ...
    description: currentCat?.description ?? "Articoli e guide di crescita personale su NorthStar.",
  ...
  const allTags = Array.from(new Set(allArticles.flatMap(a => a.tags))).slice(0, 12);
      ...
      <div className="container mx-auto px-4 md:px-6 max-w-6xl py-10">
        <div className="flex gap-8">
          ...
          <aside className="hidden lg:block w-56 shrink-0 space-y-6">
            ...
            <div className="rounded-2xl border bg-card p-4">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">Altre aree</p>
              <div className="space-y-1.5">
                {catData.filter(c => c.id !== cat && c.count > 0).slice(0, 6).map(c => (
