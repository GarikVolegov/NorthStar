...
export default function CrescitaArticolo() {
  ...
  usePageMeta({
    title: article
      ? `${article.title} — Crescita Personale — NorthStar`
      : "Articolo — NorthStar",
    ...
    return (
      <div className="container mx-auto px-4 md:px-6 max-w-3xl py-20 text-center">
        <p className="text-muted-foreground">
          {t("growth.articleNotFound", {
            defaultValue: "Articolo non trovato.",
          })}
        </p>
        <Link href="/crescita">
          <Button variant="outline" className="mt-4 rounded-full">
            ←{" "}
            {t("growth.backToLibrary", { defaultValue: "Torna alla libreria" })}
          </Button>
        </Link>
      </div>
    );
    ...
    <div className="min-h-screen pb-20">
      ...
      <div className="border-b bg-muted/30">
        <div className="container mx-auto px-4 md:px-6 max-w-4xl py-3 flex items-center gap-2 text-sm text-muted-foreground flex-wrap">
          ...
          >
            {t("growth.title", { defaultValue: "Crescita Personale" })}
          </Link>
          <span>/</span>
          <Link
            href={`/crescita/categoria/${article.category}`}
        ...
        <div className="flex gap-10">
            ...
            <div className="mb-8">
              <div className="flex flex-wrap items-center gap-2 mb-4">
                ...
                <span className="text-xs text-muted-foreground flex items-center gap-1">
                  <Clock className="w-3 h-3" /> {article.readTimeMinutes} min di
                  lettura
                </span>
                <span className="text-xs text-muted-foreground flex items-center gap-1">
              ...
              <div className="flex items-center justify-between mt-6 pt-4 border-t flex-wrap gap-3">
                <div className="flex items-center gap-3">
                  ...
                  <TTSButton
                    text={`${article.title}. ${article.description}. ${article.content.replace(/#{1,6}\s|[*_~`]/g, "")}`}
                  />
                </div>
                <Button
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {article.personalityMatches.map((p) => (
                        <span
                          key={p}
                          className="text-xs bg-violet-100 text-violet-700 px-2.5 py-1 rounded-full capitalize"
                        >
                          {p}
                        </span>
                      ))}
            ...
            {!isLoggedIn && (
              <div className="mt-8 rounded-2xl border bg-primary/5 border-primary/20 p-6 text-center">
                <p className="font-semibold text-foreground mb-1">
                  {t("growth.saveArticleTitle", {
                    defaultValue: "Salva questo articolo",
                  })}
                </p>
                <p className="text-sm text-muted-foreground mb-4">
                  {t("growth.saveArticleDesc", {
                    defaultValue:
                      "Accedi per salvare i contenuti e ritrovarli nel tuo profilo.",
                  })}
                </p>
                <Link href="/test">
                  <Button className="rounded-full">
                    {t("growth.startJourney", {
                      defaultValue: "Inizia il percorso",
                    })}
                  </Button>
                </Link>
              </div>
          ...
          <aside className="hidden lg:block w-64 shrink-0 space-y-5 pt-1">
            ...
            <div className="rounded-2xl border bg-card p-4">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">
                {t("growth.actions", { defaultValue: "Azioni" })}
              </p>
              <Button
                variant={saved ? "default" : "outline"}
                size="sm"
              ...
              >
                ...
                {saved
                  ? t("growth.articleSaved", {
                      defaultValue: "Articolo salvato",
                    })
                  : t("growth.saveArticleBtn", {
                      defaultValue: "Salva articolo",
                    })}
              </Button>
              {!isLoggedIn && (
                <p className="text-xs text-muted-foreground mt-2 text-center">
                  {t("growth.loginToSave", {
                    defaultValue: "Accedi per salvare",
                  })}
                </p>
              )}
            </div>
            ...
            <div className="rounded-2xl border bg-card p-4 space-y-3">
              <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">
                {t("growth.details", { defaultValue: "Dettagli" })}
              </p>
              <div className="space-y-2 text-sm">
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                    {t("growth.category", { defaultValue: "Categoria" })}
                  </span>
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                    {t("growth.level", { defaultValue: "Livello" })}
                  </span>
                ...
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                    {t("growth.reading", { defaultValue: "Lettura" })}
                  </span>
                  <span className="font-medium">
            ...
            {article.related.length > 0 && (
              <div className="rounded-2xl border bg-card p-4">
                <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wide mb-3">
                  {t("growth.readAlso", { defaultValue: "Leggi anche" })}
                </p>
                ...
                <Link href={`/crescita/categoria/${article.category}`}>
                  <div className="flex items-center gap-1 text-xs text-primary mt-3 pt-3 border-t hover:underline cursor-pointer">
                    {t("growth.allArticles", {
                      defaultValue: "Tutti gli articoli",
                    })}{" "}
            ...
            <div className="rounded-2xl border bg-primary/5 border-primary/20 p-4 text-center">
              <p className="text-sm font-semibold text-foreground mb-2">
                {t("growth.discoverProfile", {
                  defaultValue: "Scopri il tuo profilo",
                })}
              </p>
              <p className="text-xs text-muted-foreground mb-3">
                {t("growth.discoverProfileDesc", {
                  defaultValue:
                    "Fai il test RIASEC per trovare la direzione giusta.",
                })}
              </p>
              <Link href="/test">
