...
function useAdminFetch<T>(tab: Tab, adminKey: string) {
  ...
  const fetch_ = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const res = await apiFetch(`${BASE}api/admin/catalogs/${tab}`, {
...
function SectorForm({
  ...
  async function save() {
    ...
    try {
      const url = initial
        ? `${BASE}api/admin/catalogs/sectors/${initial.id}`
        : `${BASE}api/admin/catalogs/sectors`;
        ...
        <div className="grid grid-cols-3 gap-3">
          <div>
            ...
            <select
              ...
              onChange={(e) => setAutomationRisk(e.target.value)}
          ...
          <div>
            ...
            >
              ...
              <option value="booming">Boom</option>
            </select>
          </div>
          <div>
            <label className="text-xs font-medium text-muted-foreground">
...
function ProfessionForm({
  ...
  const [salaryRange, setSalaryRange] = useState(
    initial?.salaryRange ?? "25.000â€“45.000 â‚¬",
  );
  ...
  async function save() {
    ...
    try {
      const url = initial
        ? `${BASE}api/admin/catalogs/professions/${initial.id}`
        : `${BASE}api/admin/catalogs/professions`;
      const method = initial ? "PUT" : "POST";
      await fetch(url, {
        method,
        headers: {
      ...
      <CardContent className="pt-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          <div>
            ...
            <select
              value={growthOutlook}
              onChange={(e) => setGrowthOutlook(e.target.value)}
              className="mt-1 w-full text-sm border rounded-md px-2 py-1.5 bg-background"
            >
              <option value="declining">In calo</option>
              <option value="stable">Stabile</option>
...
function EducationForm({
  ...
  async function save() {
    ...
    try {
      const url = initial
        ? `${BASE}api/admin/catalogs/education-paths/${initial.id}`
        : `${BASE}api/admin/catalogs/education-paths`;
      const method = initial ? "PUT" : "POST";
      await fetch(url, {
        method,
        headers: {
...
function GrowthArticleForm({
  ...
  const readTime = String(initial?.readTimeMinutes ?? 3);
  ...
  async function save() {
    ...
    try {
      const url = initial
        ? `${BASE}api/admin/catalogs/growth-articles/${initial.id}`
        : `${BASE}api/admin/catalogs/growth-articles`;
      const method = initial ? "PUT" : "POST";
      await fetch(url, {
        method,
        headers: {
  ...
  return (
    <div className="flex items-center justify-between gap-3 p-3 rounded-lg hover:bg-muted/50 border border-transparent hover:border-border transition-colors">
      ...
      {active !== undefined && (
        <Badge
          variant="outline"
          className={`text-xs shrink-0 ${active ? "text-emerald-600 border-emerald-200" : "text-muted-foreground"}`}
        >
...
export default function AdminCataloghi() {
  ...
  const resources = {
    sectors,
    professions,
    "education-paths": educationPaths,
    "growth-articles": growthArticles,
  };
          ...
          <Card>
            <CardContent className="pt-4">
              ...
              <div className="space-y-1">
                  ...
                  (sectors.data as Sector[]).map((s) =>
                    editingId === s.id ? (
                      ...
                      <ListRow
                        key={s.id}
                        label={s.name}
                        sub={`${s.trend} Â· rischio ${s.automationRisk}`}
                        onEdit={() => setEditingId(s.id)}
                        onDelete={() =>
                          deleteItem(
                            `${BASE}api/admin/catalogs/sectors/${s.id}`,
                          )
                        }
                      />
                    ),
                ...
                {tab === "professions" &&
                  (professions.data as Profession[]).map((p) =>
                    editingId === p.id ? (
                      <ProfessionForm
                        key={p.id}
                        initial={p}
                      ...
                      <ListRow
                        key={p.id}
                        label={p.title}
                        sub={`${p.sector} Â· ${p.salaryRange}`}
                        active={p.isActive}
                        onEdit={() => setEditingId(p.id)}
                        onDelete={() =>
                          deleteItem(
                            `${BASE}api/admin/catalogs/professions/${p.id}`,
                          )
                        }
                      />
                    ),
                ...
                {tab === "education-paths" &&
                  (educationPaths.data as EducationPath[]).map((e) =>
                    editingId === e.id ? (
                      <EducationForm
                        key={e.id}
                        initial={e}
                      ...
                      <ListRow
                        key={e.id}
                        label={e.path}
                        sub={`${e.type} Â· ${e.duration} Â· ${e.cost}`}
                        active={e.isActive}
                        onEdit={() => setEditingId(e.id)}
                        onDelete={() =>
                          deleteItem(
                            `${BASE}api/admin/catalogs/education-paths/${e.id}`,
                          )
                        }
                      />
                    ),
                ...
                {tab === "growth-articles" &&
                  (growthArticles.data as GrowthArticle[]).map((a) =>
                    editingId === a.id ? (
                      <GrowthArticleForm
                        key={a.id}
                        initial={a}
                      ...
                      <ListRow
                        key={a.id}
                        label={a.title}
                        sub={`${a.category} Â· ${a.difficulty} Â· ${a.status}`}
                        onEdit={() => setEditingId(a.id)}
                        onDelete={() =>
                          deleteItem(
                            `${BASE}api/admin/catalogs/growth-articles/${a.id}`,
                          )
                        }
                      />
                    ),
