...
import { CheckCircle2, Circle, Share2, Star, Copy, Check, ArrowLeft } from "lucide-react";
...
interface ScoreStep {
  ...
  points: number; earned: number; done: boolean;
}

interface JourneyScore {
  userId: number; name: string;
  score: number; level: string; levelEmoji: string;
  steps: ScoreStep[];
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <h1 className="text-2xl font-bold text-foreground">Score non disponibile</h1>
        <p className="text-muted-foreground">Questo profilo è privato o non esiste.</p>
        <Link href="/"><Button variant="outline" className="rounded-full gap-2"><ArrowLeft className="h-4 w-4" /> Torna alla home</Button></Link>
      </div>
    );
  }
  ...
  if (!data.isPublic) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center gap-4 text-center px-4">
        ...
        <p className="text-muted-foreground">Questo utente ha mantenuto privato il suo percorso.</p>
        <Link href="/"><Button variant="outline" className="rounded-full gap-2"><ArrowLeft className="h-4 w-4" /> Scopri NorthStar</Button></Link>
      </div>
    );
  }

  const levelColor =
    data.score >= 85 ? "text-primary" :
    data.score >= 65 ? "text-primary" :
    data.score >= 45 ? "text-amber-400" :
    "text-orange-400";
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <div className="relative overflow-hidden bg-card border-b border-border">
        ...
        <div className="container mx-auto px-4 max-w-2xl py-16 text-center relative">
          ...
          <div className="flex items-center justify-center gap-3">
            ...
            <Button variant="outline" onClick={copyLink} className="rounded-full gap-2">
      ...
      <div className="container mx-auto px-4 max-w-2xl py-10">
        <h2 className="text-lg font-bold text-foreground mb-5">Dettaglio percorso</h2>
          ...
          {data.steps.map((step) => (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className={cn("font-semibold text-sm", step.done ? "text-foreground" : "text-muted-foreground")}>
                  {step.label}
                </div>
                <div className="text-xs text-muted-foreground">{step.description}</div>
              </div>
              <div className={cn("text-sm font-bold shrink-0", step.done ? "text-primary" : "text-muted-foreground/40")}>
        ...
        <div className="mt-12 rounded-2xl border border-primary/20 bg-primary/5 p-8 text-center">
          ...
          <h3 className="text-xl font-bold text-foreground mb-2">Scopri il tuo NorthStar Score</h3>
          <p className="text-muted-foreground text-sm mb-6">
            Orientamento professionale guidato dall'AI. Test RIASEC, settori, obiettivi e molto altro.
          </p>
          <Link href="/registra">
