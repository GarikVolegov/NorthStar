...
router.get("/weak-signals", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    const rows = await db
      ...
      .where(
        status !== "all" && isOneOf(status, WEAK_SIGNAL_STATUSES)
...
router.patch(
  ...
  async (req, res) => {
    ...
    try {
      ...
      if (!updated) {
        res.status(404).json({ error: "Segnale non trovato" });
        return;
      }
      log.info(
        { id, patch: parsed.data, adminId: req.user?.id },
        "[rag-admin] weak signal patched",
      );
  ...
  async (req, res) => {
    ...
    try {
      ...
      log.info(
        ...
        "[rag-admin] weak signal approved → confirmed",
      );
      res.json({ ok: true });
    } catch (e) {
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      log.info(
        { id, adminId: req.user?.id },
        "[rag-admin] weak signal dismissed → faded",
      );
      res.json({ ok: true });
    } catch (e) {
...
router.post("/rag-sources", requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    ...
    if (!row) {
      throw new Error("Fonte RAG non creata");
    }

    log.info(
      { id: row.id, name: row.name, adminId: req.user?.id },
      "[rag-admin] rag source created",
    );
...
router.post(
  ...
  async (req, res) => {
    ...
    try {
      ...
      log.info(
        { sourceId, ...result, adminId: req.user?.id },
        "[rag-admin] PDF ingested",
      );
...
router.post(
  ...
  async (req, res) => {
    ...
    if (source.format !== "rss") {
      res.status(400).json({ error: "Questa fonte non è di formato RSS" });
      return;
    }
    if (!source.url) {
      res.status(400).json({ error: "URL mancante per la fonte RSS" });
      return;
    }
