...
import { routerAgent } from "@workspace/ai-server/growth-agent/router-agent";
import { getSpecialist } from "@workspace/ai-server/growth-agent/specialist-agent";
import type { UserContext } from "@workspace/ai-server/growth-agent/prompt-builder";
import { retrieve } from "@workspace/ai-server/growth-agent/retriever";
import {
  loadMemory,
  buildMemorySection,
} from "@workspace/ai-server/growth-agent/memory-manager";
...
async function streamUnauthenticatedResponse(res: Response): Promise<void> {
  ...
  res.write(
    `data: ${JSON.stringify({ type: "done", model: "static", reason: "unauthenticated" })}\n\n`,
  );
  res.end();
}
  ...
  async (req: Request, res: Response) => {
    ...
    try {
      ...
      const contextSection =
        chunks.length > 0
          ? `\n\n## Contesto recuperato dalla knowledge base\n${chunks
              .map(
                (c, i) =>
                  `[FONTE ${i + 1}] (${c.source}, score: ${c.score.toFixed(2)})\n${c.content}`,
              )
              .join("\n\n")}`
          : "";
      ...
      if (specialist) {
        ...
        const specialistOpts: Parameters<typeof specialist.run>[0] = {
          userId,
          userContext: {
            ...userContext,
            memorySection,
        ...
        for await (const event of specialist.run(specialistOpts)) {
          switch (event.type) {
            case "status":
              res.write(
                `data: ${JSON.stringify({ type: "status", value: event.value })}\n\n`,
              );
              break;
            case "token":
              res.write(
                `data: ${JSON.stringify({ type: "token", content: event.value })}\n\n`,
              );
        ...
        log.warn(
          ...
          "no specialist found, falling back to general Wendy",
        );
...
Se non è sufficiente, dillo e chiedi più contesto. Non inventare informazioni.`;
      ...
      res.write(
        `data: ${JSON.stringify({ type: "error", message: "Errore durante la generazione della risposta" })}\n\n`,
      );
      res.end();
    }
  },
