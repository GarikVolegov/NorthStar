...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".northstar-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("NORTHSTAR_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("NORTHSTAR_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "northstar-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.northstar-pp-cli/feedback.jsonl.
When ` + "`NORTHSTAR_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`NORTHSTAR_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  northstar-pp-cli feedback list
  northstar-pp-cli feedback list --limit 5
  northstar-pp-cli feedback list --json`,
