...
In local mode: searches locally synced data only.`,
  ...
  northstar-pp-cli search "error timeout"

  # Force local search only
  northstar-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  northstar-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  northstar-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'northstar-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			case "sectors":
				results, err = db.SearchSectors(query, limit)
			case "":
				...
				{
					partial, searchErr := db.SearchSectors(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search sectors failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/northstar-pp-cli/data.db)")
