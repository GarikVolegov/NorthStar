...
Exit codes & warnings:
  ...
  northstar-pp-cli sync

  # Sync specific resources only
  northstar-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  northstar-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  northstar-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  northstar-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  northstar-pp-cli sync --latest-only`,
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "sectors":
		return db.UpsertSectors(data)
	case "sectors_roles":
		return db.UpsertSectorsRoles(data)
	case "sectors_stats":
		return db.UpsertSectorsStats(data)
	case "test-sessions":
		return db.UpsertTestSessions(data)
	case "confirm":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"calendar-upcoming": "/calendar/upcoming",
