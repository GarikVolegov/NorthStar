...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  northstar-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  northstar-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"calendar", "calendar-upcoming", "notifications", "sectors"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  northstar-pp-cli workflow status

  # Show status as JSON
  northstar-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("northstar-pp-cli")
			}
