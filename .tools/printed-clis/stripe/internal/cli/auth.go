...
import (
	...
	"os/exec"
	"runtime"
	"stripe-pp-cli/internal/cliutil"
	"stripe-pp-cli/internal/config"
)
...
func newAuthSetupCmd(_ *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: "  stripe-pp-cli auth setup\n  stripe-pp-cli auth setup --launch",
		RunE: func(cmd *cobra.Command, args []string) error {
			w := cmd.OutOrStdout()
			fmt.Fprintln(w, "Get a key at: https://dashboard.stripe.com/apikeys")
			fmt.Fprintln(w, "  Use a test mode key (sk_test_*) unless you intend to charge live cards.")
			fmt.Fprintln(w, "")
			fmt.Fprintln(w, "Then set:")
			fmt.Fprintln(w, "  export STRIPE_BEARER_AUTH=\"<your-token>\"")
			fmt.Fprintln(w, "  stripe-pp-cli auth set-token <token>")
			if !launch {
				return nil
			}
			launchURL := "https://dashboard.stripe.com/apikeys"
	...
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if !authed {
				...
				fmt.Fprintln(w, "  export STRIPE_BEARER_AUTH=\"your-token-here\"")
...
func newAuthLogoutCmd(flags *rootFlags) *cobra.Command {
	return &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			envStillSet := ""
			if envStillSet == "" && os.Getenv("STRIPE_BEARER_AUTH") != "" {
				envStillSet = "STRIPE_BEARER_AUTH"
			}
