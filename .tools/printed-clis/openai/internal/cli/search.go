...
In local mode: searches locally synced data only.`,
  ...
  openai-pp-cli search "error timeout"

  # Force local search only
  openai-pp-cli search "payment failed" --data-source local

  # Search a specific resource type locally
  openai-pp-cli search "critical" --type transactions --data-source local

  # JSON output for piping
  openai-pp-cli search "critical" --json --limit 20`,
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			query := args[0]
			// This API has a search endpoint: POST /vector_stores/{vector_store_id}/search
			if flags.dataSource != "local" {
				c, err := flags.newClient()
				if err != nil {
					return err
				}
				data, _, getErr := c.Post("/vector_stores/{vector_store_id}/search", map[string]any{
					"query":           query,
					"filters":         "",
					"max_num_results": 10,
			...
			if err != nil {
				return fmt.Errorf("opening local database: %w\nRun 'openai-pp-cli sync' first to populate the local database.", err)
			}
			defer db.Close()
			...
			switch resourceType {
			case "assistants":
				results, err = db.SearchAssistants(query, limit)
			case "evals":
				results, err = db.SearchEvals(query, limit)
			case "fine-tuning":
				results, err = db.SearchFineTuning(query, limit)
			case "organization":
				results, err = db.SearchOrganization(query, limit)
			case "skills":
				results, err = db.SearchSkills(query, limit)
			case "vector-stores":
				results, err = db.SearchVectorStores(query, limit)
			case "":
				// Search all FTS-enabled tables individually to avoid duplicates.
				seen := make(map[string]bool)
				_ = seen // prevent unused error when no FTS tables exist
				{
					partial, searchErr := db.SearchAssistants(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search assistants failed: %w", searchErr)
					}
				...
				{
					partial, searchErr := db.SearchEvals(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search evals failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchFineTuning(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search fine_tuning failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchOrganization(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search organization failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
				...
				{
					partial, searchErr := db.SearchVectorStores(query, limit)
					if searchErr != nil {
						return fmt.Errorf("search vector_stores failed: %w", searchErr)
					}
					for _, r := range partial {
						key := string(r)
						if !seen[key] {
