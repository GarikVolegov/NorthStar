...
Exit codes & warnings:
  ...
  openai-pp-cli sync --resources channels,messages

  # Full resync (ignore previous checkpoint)
  openai-pp-cli sync --full

  # Incremental sync: only records from the last 7 days
  openai-pp-cli sync --since 7d

  # Parallel sync with 8 workers
  openai-pp-cli sync --concurrency 8

  # Latest-only: refresh head of each resource, no historical backfill
  openai-pp-cli sync --latest-only`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if dbPath == "" {
				dbPath = defaultDBPath("openai-pp-cli")
			}
	...
	cmd.Flags().StringVar(&dbPath, "db", "", "Database path (default: ~/.local/share/openai-pp-cli/data.db)")
...
func upsertSingleObject(db *store.Store, resource string, data json.RawMessage) error {
	...
	case "batches_cancel":
		return db.UpsertBatchesCancel(data)
	case "chat":
		return db.UpsertChat(data)
	case "chatkit":
		return db.UpsertChatkit(data)
	case "completions":
	...
	case "evals":
		return db.UpsertEvals(data)
	case "evals_runs":
		return db.UpsertEvalsRuns(data)
	case "files":
		return db.UpsertFiles(data)
	case "files_content":
		return db.UpsertFilesContent(data)
	case "fine-tuning":
		return db.UpsertFineTuning(data)
	case "images":
	...
	case "responses_cancel":
		return db.UpsertResponsesCancel(data)
	case "input_items":
	...
	case "threads_runs":
		return db.UpsertThreadsRuns(data)
	case "uploads":
		return db.UpsertUploads(data)
	case "uploads_cancel":
		return db.UpsertUploadsCancel(data)
	case "complete":
		return db.UpsertComplete(data)
	case "parts":
		return db.UpsertParts(data)
	case "vector-stores":
		return db.UpsertVectorStores(data)
	case "file_batches":
		return db.UpsertFileBatches(data)
	case "vector_stores_files":
		return db.UpsertVectorStoresFiles(data)
	case "search":
...
func syncResourcePath(resource string) (string, error) {
	paths := map[string]string{
		...
		"audio":                       "/audio/voice_consents",
		"batches":                     "/batches",
		"chat":                        "/chat/completions",
		"chatkit":                     "/chatkit/threads",
		...
		"organization-admin-api-keys": "/organization/admin_api_keys",
		"organization-audit-logs":     "/organization/audit_logs",
...
func dependentResourceDefs() []dependentResourceDef {
	return []dependentResourceDef{
		{Name: "containers_files", ParentTable: "containers", ParentIDParam: "container_id", PathTemplate: "/containers/{container_id}/files", KeyField: ""},
		{Name: "evals_runs", ParentTable: "evals", ParentIDParam: "eval_id", PathTemplate: "/evals/{eval_id}/runs/{run_id}/output_items", KeyField: ""},
		{Name: "file_batches", ParentTable: "vector-stores", ParentIDParam: "vector_store_id", PathTemplate: "/vector_stores/{vector_store_id}/file_batches/{batch_id}/files", KeyField: ""},
		{Name: "vector_stores_files", ParentTable: "vector-stores", ParentIDParam: "vector_store_id", PathTemplate: "/vector_stores/{vector_store_id}/files", KeyField: ""},
		{Name: "versions", ParentTable: "skills", ParentIDParam: "skill_id", PathTemplate: "/skills/{skill_id}/versions", KeyField: ""},
	}
}
