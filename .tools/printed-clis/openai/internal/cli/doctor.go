...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openai-pp-cli doctor
  openai-pp-cli doctor --json
  openai-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export OPENAI_API_KEY_AUTH=<your-key>"
					report["auth_docs_url"] = "https://help.openai.com/"
				} else {
					authConfigured = true
			...
			if os.Getenv("OPENAI_API_KEY_AUTH") != "" {
				authEnvSet = append(authEnvSet, "OPENAI_API_KEY_AUTH")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "OPENAI_API_KEY_AUTH")
			}
			switch {
...
func collectCacheReport(ctx context.Context, staleAfterSpec string) map[string]any {
	...
	if err != nil {
		if os.IsNotExist(err) {
			...
			report["hint"] = "Database not created yet; run 'openai-pp-cli sync' to hydrate."
			return report
		}
		report["status"] = "error"
	...
	if qerr != nil {
		...
		report["status"] = "unknown"
		report["hint"] = "No sync state recorded; run 'openai-pp-cli sync' to populate."
		return report
	}
	defer rows.Close()
	...
	case !haveAny && len(resources) == 0:
		report["status"] = "unknown"
		report["hint"] = "sync_state is empty; run 'openai-pp-cli sync' to hydrate."
	case fresh:
		report["status"] = "fresh"
	default:
		report["status"] = "stale"
		report["oldest_age"] = oldest.Round(time.Minute).String()
		report["hint"] = "Some resources are older than stale_after; run 'openai-pp-cli sync' to refresh."
	}
	return report
}
