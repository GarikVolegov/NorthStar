...
and full resync. After archiving, use 'search' for instant full-text search.`,
  ...
  openai-pp-cli workflow archive

  # Full re-archive (ignore previous sync state)
  openai-pp-cli workflow archive --full`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			resources := []string{"assistants", "audio", "batches", "chat", "chatkit", "containers", "evals", "files", "fine-tuning", "models", "organization", "organization-admin-api-keys", "organization-audit-logs", "organization-certificates", "organization-groups", "organization-invites", "organization-projects", "organization-users", "skills", "vector-stores", "videos"}
			totalSynced := 0
...
func newWorkflowStatusCmd(flags *rootFlags) *cobra.Command {
  ...
  openai-pp-cli workflow status

  # Show status as JSON
  openai-pp-cli workflow status --json`,
		RunE: func(cmd *cobra.Command, args []string) error {
			if dbPath == "" {
				dbPath = defaultDBPath("openai-pp-cli")
			}
