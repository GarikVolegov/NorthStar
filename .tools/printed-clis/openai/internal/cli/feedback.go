...
func feedbackFilePath() (string, error) {
	...
	dir := filepath.Join(home, ".openai-pp-cli")
...
func FeedbackEndpointConfigured() bool {
	return os.Getenv("OPENAI_FEEDBACK_ENDPOINT") != ""
}
...
func feedbackAutoSend() bool {
	v := strings.ToLower(strings.TrimSpace(os.Getenv("OPENAI_FEEDBACK_AUTO_SEND")))
...
func postFeedback(url string, entry FeedbackEntry) error {
	...
	req.Header.Set("User-Agent", "openai-pp-cli/feedback")
...
func newFeedbackCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long: `Feedback is captured locally first at ~/.openai-pp-cli/feedback.jsonl.
When ` + "`OPENAI_FEEDBACK_ENDPOINT`" + ` is set and either --send is
passed or ` + "`OPENAI_FEEDBACK_AUTO_SEND=true`" + `, the entry is
...
func newFeedbackListCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  openai-pp-cli feedback list
  openai-pp-cli feedback list --limit 5
  openai-pp-cli feedback list --json`,
