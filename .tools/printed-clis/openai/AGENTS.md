# Openai Printed CLI Agent Guide

This directory is a generated `openai-pp-cli` printed CLI. It was produced by [CLI Printing Press](https://github.com/mvanhorn/cli-printing-press), so treat systemic fixes as upstream Printing Press fixes first. Keep local edits narrow and document why a generated-tree patch belongs here.
...
```bash
openai-pp-cli doctor --json
openai-pp-cli agent-context --pretty
```

Use runtime discovery instead of relying on a copied command list:

```bash
openai-pp-cli which "<capability>" --json
openai-pp-cli <command> --help
```
...
```bash
openai-pp-cli <command> --help
openai-pp-cli <command> --dry-run --agent
```
