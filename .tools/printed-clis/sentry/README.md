...
```bash
sentry-pp-cli auth set-token YOUR_TOKEN_HERE
```

Or set it via environment variable:

```bash
export REFERENCE_AUTH_TOKEN="your-token-here"
```
...
```bash
claude mcp add sentry sentry-pp-mcp -e REFERENCE_AUTH_TOKEN=<your-token>
```
...
3. Fill in `REFERENCE_AUTH_TOKEN` when Claude Desktop prompts you.
...
Config file: `~/.config/reference-pp-cli/config.toml`
...
| --- | --- | --- | --- |
| `REFERENCE_AUTH_TOKEN` | per_call | Yes | Set to your API credential. |

## Troubleshooting
**Authentication errors (exit code 4)**
- Run `sentry-pp-cli doctor` to check credentials
- Verify the environment variable is set: `echo $REFERENCE_AUTH_TOKEN`
