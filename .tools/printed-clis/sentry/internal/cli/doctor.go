...
func newDoctorCmd(flags *rootFlags) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Example: `  sentry-pp-cli doctor
  sentry-pp-cli doctor --json
  sentry-pp-cli doctor --fail-on warn`,
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if cfg != nil {
				...
				if header == "" {
					...
					report["auth_hint"] = "export REFERENCE_AUTH_TOKEN=<your-key>"
					report["auth_key_url"] = "https://sentry.io/settings/account/api/auth-tokens/"
					report["auth_instructions"] = "Create an auth token with the scopes your workflow needs (project:read, event:read, etc.)."
				} else {
					authConfigured = true
			...
			if os.Getenv("REFERENCE_AUTH_TOKEN") != "" {
				authEnvSet = append(authEnvSet, "REFERENCE_AUTH_TOKEN")
				...
				authEnvRequiredMissing = append(authEnvRequiredMissing, "REFERENCE_AUTH_TOKEN")
			}
			switch {
