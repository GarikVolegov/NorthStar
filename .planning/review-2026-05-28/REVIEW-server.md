...
- Proposta: confermare che CSRF non sia un vettore (header `Authorization` + CORS ristretto) e documentarlo in [SECURITY.md](SECURITY.md). Non un fix di codice, una validazione.
- Costo: 30 min review.
...
- Proposta: leggere entrambe le implementazioni; se sono equivalenti, eliminare il duplicato e standardizzare su `requireAdmin`. Se sono diverse, documentare in CLAUDE.md il perché.
- Costo: 20 min.
