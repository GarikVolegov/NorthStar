...
export default function CertificatePage() {
  ...
  const { data, isLoading, isError } = useQuery<VerifyResult>({
    ...
    queryFn: async () => {
      const r = await fetch(`${BASE}api/nft-certificates/verify/${hash}`);
  ...
  return (
    <div className="min-h-screen bg-background">
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur border-b border-border/50 px-6 py-3 flex items-center justify-between">
        ...
        <Link href="/">
          <Button variant="ghost" size="sm" className="rounded-full text-xs">← Home</Button>
        </Link>
      </nav>
      ...
      <div className="pt-20 pb-16 px-4 max-w-3xl mx-auto">
        ...
        <div className="text-center mb-10">
          ...
          <p className="text-muted-foreground text-sm">
            Ogni certificato NorthStar è firmato crittograficamente e verificabile pubblicamente
          </p>
        </div>
        ...
        {isError && (
          <div className="rounded-2xl border border-destructive/20 bg-destructive/5 p-10 text-center space-y-4">
            ...
            <div>
              <h2 className="font-bold text-foreground text-lg">Certificato non trovato</h2>
              <p className="text-muted-foreground text-sm mt-1">
                L'hash fornito non corrisponde a nessun certificato emesso da NorthStar.
              </p>
            </div>
            <Link href="/">
        ...
        {cert && (
          <div className="space-y-6">
            ...
            <div className="rounded-2xl border border-green-500/20 bg-green-500/5 p-4 flex items-center gap-3">
              ...
              <div>
                <p className="font-bold text-green-400 text-sm">Certificato autentico e verificato</p>
                <p className="text-xs text-muted-foreground">
                  Hash SHA-256 corrispondente · Emesso da NorthStar · {cert.chain}
                </p>
              </div>
              <div className="ml-auto flex items-center gap-1.5 text-xs">
                <span className="w-2 h-2 rounded-full bg-green-400 animate-pulse" />
                <span className="text-green-400 font-semibold">{cert.status.toUpperCase()}</span>
              </div>
            </div>
            ...
            <div
              ...
              style={{ borderColor: `${colors.accent}25`, background: colors.bg }}
            >
              <div className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <Gem className="h-5 w-5" style={{ color: colors.accent }} />
                  <h2 className="font-bold text-foreground text-lg">Dettagli certificato</h2>
                </div>
                <span
                  className="text-xs font-bold tracking-widest uppercase px-3 py-1 rounded-full"
                  style={{ color: colors.accent, background: `${colors.accent}15`, border: `1px solid ${colors.accent}30` }}
                >
                  {label}
                </span>
              </div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                {[
                  ...
                  { icon: Clock, label: "Data certificazione", value: new Date(cert.mintedAt).toLocaleDateString("it-IT", { day: "2-digit", month: "long", year: "numeric" }) },
                  { icon: Gem, label: "Chain", value: cert.chain },
                  { icon: CheckCircle2, label: "Status", value: cert.status },
                ].map(({ icon: Icon, label: lbl, value }) => (
                  <div key={lbl} className="rounded-xl border border-border/60 bg-background/60 p-3.5">
                    <div className="flex items-center gap-2 mb-1">
                      <Icon className="h-3.5 w-3.5 text-muted-foreground" />
                      <span className="text-xs text-muted-foreground uppercase tracking-wide">{lbl}</span>
                    </div>
              ...
              <div className="rounded-xl border border-border/60 bg-background/60 p-4">
                ...
                <p className="font-semibold text-foreground leading-snug">"{cert.objectiveText}"</p>
              </div>
              ...
              <div className="rounded-xl border border-border/60 bg-background/60 p-4">
                ...
                <p className="font-mono text-xs text-muted-foreground break-all leading-relaxed">{cert.certificateHash}</p>
              </div>

              {/* Actions */}
              <div className="flex flex-wrap gap-2 pt-1">
                <CopyButton text={cert.certificateHash} label="Copia hash" />
                <CopyButton text={window.location.href} label="Copia link verifica" />
                <a href={imageUrl} download={`northstar-cert-${cert.id}.png`}>
                  <Button variant="outline" size="sm" className="rounded-lg gap-1.5 text-sm h-9">
            ...
            <p className="text-center text-xs text-muted-foreground">
              ...
              <Link href="/"><span className="text-primary cursor-pointer hover:underline">NorthStar</span></Link>{" "}
              · Piattaforma di orientamento professionale italiana ·{" "}
              <span className="font-mono">{cert.certificateHash.slice(0, 12)}…</span>
            </p>
          </div>
        )}
      </div>
