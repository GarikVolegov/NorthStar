/**
 * GrowthProfilePage v3 — prop onRetakeTest aggiunta (Passo 3).
 *
 * CHANGES vs v2:
 * - onRetakeTest prop passata a ProfileEditPanel
 * - Suggerimento nel footer sidebar aggiornato
 */
...
interface AppNotification { id:number; type:string; title:string; body:string; createdAt:string; }
    ...
    <div className="flex flex-col items-center py-8 px-4 border-b border-gray-100">
      <div className="relative">
        {avatarUrl
          ...
          : <div className="w-16 h-16 rounded-2xl bg-indigo-600 flex items-center justify-center shadow-md"><span className="text-white text-xl font-bold">{initials||"N"}</span></div>
        }
...
export interface GrowthProfilePageProps {
  ...
  /** Callback per navigare alla pagina del test RIASEC */
  onRetakeTest?: () => void;
}

export function GrowthProfilePage({
  token, userName="", journeyType="generico", userMode="esplorativo",
  objectives, sectorName, apiBase="/api", className="", profileData, onRetakeTest,
}: GrowthProfilePageProps) {
  const [activeTab,     setActiveTab]     = useState<TabId>("profilo");
  const [notification,  setNotification]  = useState<AppNotification | null>(null);
  const [liveAvatarUrl, setLiveAvatarUrl] = useState<string|null|undefined>(profileData?.avatarUrl);

  useEffect(() => {
    let cancelled = false;
    (async () => {
      try {
        const res = await fetch(`${apiBase}/growth-agent/notifications`, { headers:{Authorization:`Bearer ${token}`} });
  ...
  return (
    <div className={`flex flex-col bg-gray-50 ${className}`}>
      {notification && <DigestBanner notification={notification} onDismiss={dismissNotification}/>}
      ...
      <div className="flex flex-col lg:flex-row flex-1 min-h-0 overflow-hidden">
          ...
          <div className="mt-auto p-4">
            <div className="rounded-xl bg-indigo-50 px-4 py-3">
              ...
              <p className="text-xs text-indigo-500 mt-1">
                Ogni sessione con Wendy ti assegna XP e mantiene viva la tua streak.
              </p>
            </div>
          </div>
        </aside>
        ...
        <main className="flex flex-col flex-1 min-h-0 overflow-hidden">
          <div className="lg:hidden"><MobileTabBar active={activeTab} onChange={setActiveTab}/></div>

          {/* Profilo (sempre montato) */}
          <div className={`flex-1 overflow-y-auto p-5 ${activeTab==="profilo"?"block":"hidden"}`}>
            <ProfileEditPanel
              {...sharedProps}
              initialData={{
                name:        userName,
          ...
          {/* Chat (sempre montata, SSE) */}
          <div className={`flex-1 min-h-0 ${activeTab==="chat"?"flex":"hidden"}`}>
            <GrowthChatPanel {...sharedProps} userContext={userContext} className="flex-1 rounded-none border-0 shadow-none"/>
          </div>
