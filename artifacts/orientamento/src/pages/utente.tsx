...
function formatDate(iso: string) { return new Date(iso).toLocaleDateString("it-IT", { month: "long", year: "numeric" }); }
  ...
  if (!profile) {
    return (
      <div className="min-h-[60vh] flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-muted-foreground mb-6">Questo profilo non esiste o non è accessibile.</p>
        <Button asChild variant="outline"><Link href="/amici"><ArrowLeft className="w-4 h-4 mr-2" /> Torna agli amici</Link></Button>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-muted/30">
      <div className="max-w-lg mx-auto px-4 py-10">
        ...
        <Link href="/amici" className="inline-flex items-center gap-1.5 text-sm text-muted-foreground hover:text-foreground transition-colors mb-6">
          <ArrowLeft className="w-3.5 h-3.5" /> Amici
        </Link>
        ...
        <div className="bg-background rounded-3xl border shadow-sm p-8 text-center">
          ...
          <div className="flex items-center justify-center gap-1.5 mb-4">
            {profile.isPublic
              ? <><Globe className="w-3.5 h-3.5 text-emerald-500" /><span className="text-sm text-emerald-600 font-medium">Profilo pubblico</span></>
              : <><Lock className="w-3.5 h-3.5 text-muted-foreground" /><span className="text-sm text-muted-foreground">Profilo privato</span></>}
          </div>

          {/* Member since */}
          <p className="text-xs text-muted-foreground mb-6">
            Su NorthStar da {formatDate(profile.createdAt)}
          </p>
              ...
              {isFriend ? (
                <>
                  <div className="flex items-center gap-1.5 text-sm text-emerald-600 font-medium bg-emerald-50 border border-emerald-200 px-4 py-2 rounded-full">
                    <UserCheck className="w-4 h-4" /> Siete amici
                  </div>
                ...
                <>
                  <Button variant="outline" size="sm" className="rounded-full gap-1.5 text-amber-600 border-amber-200 bg-amber-50">
                    <Clock className="w-3.5 h-3.5" /> Richiesta inviata
                  </Button>
        ...
        {!profile.canView && !isOwnProfile && (
          <div className="mt-4 bg-background rounded-2xl border p-6 text-center">
            ...
            <p className="text-sm font-medium mb-1">Contenuto privato</p>
            <p className="text-xs text-muted-foreground">
              Diventa amico di {profile.name} per vedere il suo profilo completo.
            </p>
          </div>
        )}
        ...
        {!user && (
          <div className="mt-4 bg-background rounded-2xl border p-5 text-center">
            <p className="text-sm text-muted-foreground mb-3">
              Accedi a NorthStar per inviare una richiesta di amicizia.
            </p>
            <Button asChild className="rounded-full"><Link href="/">Vai alla home</Link></Button>
          </div>
        )}
      </div>
    </div>
