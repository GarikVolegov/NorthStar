...
import {
  ...
  Plus, Minus, Zap, Brain, MapPin, Network, ArrowRight, Newspaper,
  GitCompare, Briefcase, Laptop, MessageSquare,
} from "lucide-react";
...
const GrowthChart = React.memo(function GrowthChart({ data }: { data: ChartEntry[] }) {
  return (
...
const CareerStepList = React.memo(function CareerStepList({
  steps, stepGroup, pathKey, activeColor, t,
}: CareerStepListProps) {
  const ring = activeColor === "blue"
      ...
      {steps.map((s, i) => (
        <div key={s.step} className="flex gap-4">
          <div className="flex flex-col items-center">
            ...
            {i < steps.length - 1 && <div className={cn("w-0.5 h-full mt-1", line)} />}
          </div>
          <div className="pb-8">
            <h4 className="font-semibold text-foreground mb-1">
              {t(`careerSteps.${pathKey}.${stepGroup}.${s.step}.title`, { defaultValue: s.title })}
            </h4>
            <p className="text-sm text-muted-foreground leading-relaxed">
              {t(`careerSteps.${pathKey}.${stepGroup}.${s.step}.desc`, { defaultValue: s.description })}
            </p>
          </div>
        </div>
      ))}
...
export default function Sector() {
  ...
  usePageMeta(
    sector
      ...
      : { title: t("sector.loadingTitle"), description: t("sector.loadingDesc"), noIndex: true },
  );
    ...
    <div className="container mx-auto px-4 py-8 md:py-16 max-w-5xl">
          ...
          <div className="flex flex-wrap gap-2 mb-4">
            ...
            {workPreference && workPreference !== "unknown" && (() => {
              ...
              return (
                <div
                  title={t(
                    alignment.tooltipKey,
                    alignment.tooltipModes
                      ? { modes: alignment.tooltipModes.map((m) => t(`workMode.${m}`, { defaultValue: m })).join("/") }
                      : undefined,
                  )}
                  className={cn(
      ...
      <Tabs defaultValue="overview" className="w-full">
        <TabsList className="w-full flex justify-start border-b rounded-none h-auto bg-transparent p-0 mb-8 space-x-6 overflow-x-auto">
          {(["overview", "percorso", "skills", "data", "roles"] as const).map((v) => (
            <TabsTrigger
              key={v}
              value={v}
            ...
            >
              {v === "roles" ? (
                ...
                t(`sector.tabs.${v === "percorso" ? "career" : v === "data" ? "data" : v}`)
              )}
            </TabsTrigger>
          ))}
        </TabsList>
        ...
        <TabsContent value="percorso" className="animate-in fade-in duration-500">
          <div className="max-w-3xl">
            <div className="flex items-center justify-between mb-6 flex-wrap gap-3">
              <h3 className="text-2xl font-serif font-bold">{t("sector.careerPath")}</h3>
            ...
            {stepsView === "dipendente" ? (
              <div>
                ...
                {(!sector.dipendentiSteps || sector.dipendentiSteps.length === 0) ? (
              ...
              <div>
                ...
                {(!sector.freelanceSteps || sector.freelanceSteps.length === 0) ? (
            ...
            <div className="grid md:grid-cols-5 gap-8">
              ...
              <div className="md:col-span-2 space-y-6">
                ...
                <div className="bg-primary text-primary-foreground rounded-3xl p-6 shadow-md">
                  ...
                  <p className="text-primary-foreground/80 text-sm mb-6">{t("sector.readyDesc")}</p>
