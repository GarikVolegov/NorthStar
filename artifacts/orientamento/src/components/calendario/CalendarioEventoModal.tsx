...
import { Loader2, Trash2, Bell, Crown, AlertTriangle, ExternalLink, BookOpen, Map } from "lucide-react";
...
const eventFormSchema = z.object({
  ...
  category: z.enum(["study", "training", "interview", "deadline", "task", "follow-up"] as const),
...
export function CalendarioEventoModal({ open, onOpenChange, userId, defaultDate, editingEvent, onSaved, onDeleted }: Props) {
  ...
  const { data: quotaData } = useQuery({
    ...
    queryFn: async () => {
      ...
      return res.json() as Promise<{ isPremium: boolean; eventCount: number; eventLimit: number | null }>;
    },
    enabled: !!userId,
  });
  const isPremium = !!quotaData?.isPremium;
  const allowedReminders = isPremium ? PREMIUM_REMINDER_MINUTES : FREE_REMINDER_MINUTES;
  ...
  const form = useForm<EventFormValues>({
    ...
    defaultValues: {
      ...
      startTime: timeStr,
      endDate: dateStr,
      endTime: format(new Date(defaultDate.getTime() + 60 * 60 * 1000), "HH:mm"),
      allDay: false,
      category: "task",
      priority: "medium",
      status: "todo",
  ...
  useEffect(() => {
    ...
    if (editingEvent) {
      ...
      PREMIUM_REMINDER_MINUTES.forEach((m) => { toggles[m] = false; });
      editingEvent.reminders.forEach((r) => { toggles[r.minutesBefore] = r.enabled; });
  ...
  const buildPayload = (values: EventFormValues) => {
    ...
    const reminders = PREMIUM_REMINDER_MINUTES
      .filter((m) => reminderToggles[m])
    ...
    return {
      ...
      linkedSectorId: values.linkedSectorId ? parseInt(values.linkedSectorId, 10) : null,
      reminders,
    };
  };
        ...
        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
          ...
          {selectedSector && (
            <div className={cn(
              ...
              isPremium ? "bg-primary/5 border-primary/20" : "bg-muted/40 border-muted",
            )}>
              {isPremium ? (
                <>
