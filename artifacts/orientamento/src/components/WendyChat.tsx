...
/**
 * WendyChat — componente chat completo per Wendy
 *
 * Feature:
 *   ✓ Streaming SSE — testo scorre parola per parola
 *   ✓ Thinking indicator — animazione "Wendy sta pensando…"
 *   ✓ Error recovery — messaggi errore friendly + pulsante Riprova
 *   ✓ TTS — lettura automatica risposte (toggle mute)
 *   ✓ STT — input voce via microfono (push-to-talk)
 *   ✓ Auto-scroll — segue l'output durante lo stream
 *   ✓ Keyboard shortcut — Enter invia, Shift+Enter va a capo
 *   ✓ Accessibilità — aria-live, role="log", aria-label
 *
 * Uso:
 *   <WendyChat apiUrl="/api/v1/ai/chat/stream" className="h-[600px]" />
 */
...
export function WendyChat({
  ...
  // ─── Auto-scroll al fondo durante lo stream ──────────────────────────────
  useEffect(() => {
    messagesEndRef.current?.scrollIntoView({ behavior: 'smooth' });
  }, [messages, thinking.active]);

  // ─── Aggiorna input con trascrizione STT in tempo reale ──────────────────
  useEffect(() => {
    if (stt.isListening) {
  ...
  const handleKeyDown = useCallback(
    (e: KeyboardEvent<HTMLTextAreaElement>) => {
      if (isComposingRef.current) return; // IME in corso
    ...
    <div className={['flex flex-col bg-background border border-border rounded-2xl overflow-hidden shadow-md', className].join(' ')}>
      ...
      >
        ...
        {messages.map((msg) => (
          ...
          >
            {msg.role === 'error' ? (
              /* Messaggio errore con pulsante retry */
              ...
              <div
                className={[
                  ...
                  // Cursore lampeggiante durante lo stream del messaggio corrente
      ...
      {/* Error banner connessione (non in-chat) */}
