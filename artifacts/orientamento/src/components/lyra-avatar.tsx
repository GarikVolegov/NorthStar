...
// Palette per fase
const PHASE_PALETTE = [
  { bg: "#EFF6FF", bgDark: "#1E3A5F", accent: "#3B82F6", skin: "#FDDCB5", hair: "#2D1B00" }, // Attitudini — blu
  { bg: "#F5F3FF", bgDark: "#2E1B5F", accent: "#7C3AED", skin: "#FDDCB5", hair: "#1A0A2E" }, // Profilo Interiore — viola
  { bg: "#FFFBEB", bgDark: "#451A03", accent: "#D97706", skin: "#FDDCB5", hair: "#1C1008" }, // Obiettivi — ambra
] as const;

// Curve bocca per ogni stato
const MOUTH_PATHS: Record<AvatarState, string> = {
  curious:     "M 44 68 Q 50 72 56 68",   // sorriso lieve
  reflective:  "M 44 69 Q 50 71 56 69",   // quasi neutro, calmo
  focused:     "M 44 67 Q 50 73 56 67",   // sorriso deciso
  celebrating: "M 42 66 Q 50 76 58 66",   // sorriso ampio
};

// Sopracciglia: offset Y sinistro/destro
const BROW_OFFSETS: Record<AvatarState, { lY: number; rY: number; lRotate: number; rRotate: number }> = {
  curious:     { lY: 0, rY: -3, lRotate: 0,  rRotate: -8  }, // sopracciglio dx alzato
  reflective:  { lY: 1, rY: 1,  lRotate: 0,  rRotate: 0   }, // neutri, leggermente bassi
  focused:     { lY: 0, rY: 0,  lRotate: -3, rRotate: 3   }, // leggermente inarcati
  celebrating: { lY: -3, rY: -3, lRotate: 5, rRotate: -5  }, // alzati, gioiosi
};

// Oscillazione testa (tilt)
...
export function LyraAvatar({ state, phase, reduced = false, className, size = 120 }: LyraAvatarProps) {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* ─ Capelli ─— raccolti, professionali */}
        {/* Base capelli */}
        <ellipse cx="50" cy="30" rx="22" ry="13" fill={p.hair} />
        {/* Chignon / coda raccolta */}
        ...
        {/* ─ Orecchini (dettaglio professionale) ─ */}
      ...
      {/* Dot pulsante in basso a destra — segnala che è attiva */}
      <motion.div
